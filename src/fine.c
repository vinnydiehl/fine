...
void parse_sexp(char *src)
{
    ...
    while (regexec(&list_re, src, 1, match, 0) != REG_NOMATCH)
    {
        ...
        // eval() currently modifies the list in place for the sake of
        // efficiency but this interferes with reporting, so duplicate it when
        // debugging.
#if DEBUG
        eval(strdup(list));
#else
        ...
        // the matching character in the evaluation. When no more characters
        // are available from the latter, switch to spaces so as to "clear" the
        // list and prevent it from being processed again.
...
void debug(char* sexp, regmatch_t match, char* list)
{
